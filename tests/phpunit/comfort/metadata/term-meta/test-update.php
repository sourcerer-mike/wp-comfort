...
class Update_Test extends TestCase {
	...
	public function testItCanReplaceExistingData() {
		$this->assertEquals( get_term_meta( $this->term_id, $this->meta_key, true ), $this->meta_value );

		$newValue = [ uniqid(), uniqid(), uniqid() ];

		update_term_meta( $this->term_id, $this->meta_key, $newValue );

		$this->assertEquals( get_term_meta( $this->term_id, $this->meta_key, true ), $newValue );
	}

	public function testNewMetaKeysWillBeCreated() {
		$someMeta = uniqid( 'comfort_test_' );

		$this->assertEquals( get_term_meta( $this->term_id, $someMeta, true ), null );
